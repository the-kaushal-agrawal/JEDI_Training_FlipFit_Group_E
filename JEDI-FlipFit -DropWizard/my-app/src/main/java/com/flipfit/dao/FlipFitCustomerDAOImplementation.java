...
import com.flipfit.model.FlipFitBookings;
...
public class FlipFitCustomerDAOImplementation implements FlipFitCustomerDAOInterface {
    ...
    @Override
    public List<FlipFitGym> viewAllGymsByArea(String area) {
        ...
        for (FlipFitGym gym : gyms) {
            List<FlipFitSlots> slots = flipFitGymOwnerDAOImplementation.getSlotsByGymId(gym.getGymId());
            gym.setSlots(slots);
        }
        return gyms;
    }
    ...
    @Override
    public List<FlipFitBookings> getAllBookingsByUserID(int userId) {
    ...
    public boolean cancelBooking(int bookingId) {
        ...
        if (isBookingCancelled) {
            FlipFitBookings booking = getBooking(bookingId);
    ...
    public FlipFitBookings getBooking(int bookingId) {
        FlipFitBookings booking = new FlipFitBookings();
    ...
    @Override
    public boolean createUser(FlipFitUser user) {
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            int rowsInserted = preparedStatement.executeUpdate();
            if (rowsInserted > 0) {
                // System.out.println("FlipFitUser created successfully!");
                return true;
            } else {
            ...
            // System.out.println("FlipFitUser " + e.getMessage());
            return false;
        }
    }

    public boolean updateUserDetails(FlipFitUser user) {
        try (Connection conn = DatabaseConnector.getConnection();
